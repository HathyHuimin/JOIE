...
def calc_pdparam(state, algorithm, body):
    ...
    if not torch.is_tensor(state):  # dont need to cast from numpy
        ...
        state = state.to(algorithm.device)
...
def default(state, algorithm, body, hisda):
    '''Plain policy by direct sampling from a default action probability defined by body.ActionPD'''
    if algorithm.algorithm_spec["name"] in ["WarmUpHierDQN"]:
        pdparam = algorithm.calc_pdparam(state, body)
        return pdparam
    elif algorithm.algorithm_spec["name"] in ["WDQN_Co_Jo", "WDQN_Co3_Jo"]:
        pdparam = algorithm.calc_pdparam(state, eval=util.in_eval_lab_modes())
        return pdparam
    else:
        pdparam = calc_pdparam(state, algorithm, body)
    action = sample_action(body.ActionPD, pdparam)
...
def warmup_epsilon_greedy(state, algorithm, body, hisda):
    action = default(state, algorithm, body, hisda)
    if util.in_eval_lab_modes():
        return action

    epsilon = body.explore_var
    if epsilon > np.random.rand() and algorithm.algorithm_spec["name"] not in ['WDQN_Co_Jo', "WDQN_Co3_Jo"]:
...
def rule_guide(state, algorithm, body):
    ...
    probs = torch.zeros(body.action_space.high, device=algorithm.device)
    probs[action] = 1
    action = torch.tensor(action, device=algorithm.device)
    return action
...
def multi_default(states, algorithm, body_list, pdparam):
    ...
    action_a = torch.tensor(action_list, device=algorithm.device).unsqueeze(dim=1)
    return action_a
